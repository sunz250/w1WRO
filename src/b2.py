...
while True:
    ...
    if state==0:
        bnoreg(2,4)
        if bum.getLineAnalog(1) < 2000 or bum.getLineAnalog(9) < 2000:
            whiteled.value=True
            state=1
            direction=(direction+70*clock+360)%360
            servokit.servo[0].angle = 15
            kit.motor1.throttle = 0.6 ###
    if state==1:
        if abs(direction-angle.read())<5 or area_red_banka is not None or area_green_banka is not None:
            whiteled.value=False
            #kit.motor1.throttle = 0.55
            if area_red_banka is not None: redcubled.value=True; turntype=5; treb=100; kp=0.4; kd=0.8; 
            if area_green_banka is not None: 
                greencubled.value=True
                turntype=3
                if x_green_banka>130: hardgreen=True; redled.value=True; blueled.value=True
                else: hardgreen=False; treb=640; kp=0.3; kd=0.6; time.sleep(0.3) 
                 
            direction=(direction+20*clock+360)%360
            state=2
            # time.sleep(0.3) #задержка чтобы увидел кубик
    if state==2:
        if turntype==5:
            if area_red_banka is not None:
                err=x_red_banka-treb    
                Upr = kp*err+kd*(err-lasterror)
                lasterror = err
                servoangle = servocenter+Upr+Upr*kp
                servoangle = clamp(servoangle, clamplow, clamphigh)  
                servokit.servo[0].angle = servoangle 
                x_red_prev=x_red_banka  
            else: #break#state=3
                #if hardred is None: hardred=1*(abs((angle.read()-((direction+20*clock+360)%360)))<30)
                err=(x_red_prev-treb)#*(abs((angle.read()-((direction+20*clock+360)%360)))<30)
                Upr = kp*err+kd*(err-lasterror)
                lasterror = err
                servoangle = servocenter+Upr+Upr*kp
                servoangle = clamp(servoangle, clamplow, clamphigh)  
                servokit.servo[0].angle = servoangle 
            #print((angle.read()-direction+360)%360)
        if turntype==3:
            if area_green_banka is not None and hardgreen==False:
                err=x_green_banka-treb    
                Upr = kp*err+kd*(err-lasterror)
                lasterror = err
                servoangle = servocenter+Upr+Upr*kp
                servoangle = clamp(servoangle, clamplow, clamphigh)  
                servokit.servo[0].angle = servoangle 
                x_green_prev=x_green_banka  
            if area_green_banka is None or hardgreen==True: 
                bnoreg(3,0)
                # err=(x_green_prev-treb)
                # Upr = kp*err+kd*(err-lasterror)
                # lasterror = err
                # servoangle = servocenter+Upr+Upr*kp
                # servoangle = clamp(servoangle, clamplow, clamphigh)  
                # servokit.servo[0].angle = servoangle 
                if abs(direction-angle.read())<5 : break

    #anglerr=str(abs((angle.read()-((direction+20*clock+360)%360))))
    font.OverlayText(img, 1280, 720, str(turntype), 400, 30, font.Blue, font.White)
    font.OverlayText(img, 1280, 720, str(direction), 400, 60, font.Blue, font.White)
#    font.OverlayText(img, 1280, 720, str(anglerr), 400, 90, font.Blue, font.White)
...
angle.stop()
redcubled.value=False
greencubled.value=False

#6servokit.servo[0].angle = servocenter

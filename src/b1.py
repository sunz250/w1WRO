...
while True:
    ...
    if state==0:
        bnoreg(2,4)
        if bum.getLineAnalog(1) < 1900 or bum.getLineAnalog(9) < 1900:
            whiteled.value=True
            state=1
    ...
    if state==1:
        if abs(direction-angle.read())<5 or area_red_banka is not None or area_green_banka is not None:
            ...
            #kit.motor1.throttle = 0.55
            if area_red_banka is not None: redcubled.value=True; turntype=5; treb=100; kp=0.4; kd=0.8; end = 10
            if area_green_banka is not None: greencubled.value=True; turntype=3
            state=2
    ...
    if state==2:
        ...
        if abs(((direction+20*clock+360)%360)-angle.read())<5: break
